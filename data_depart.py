...
#département selon le nombres de leurs equipements
equipement = Df.sum(axis=1,numeric_only=True)
fig1 = px.bar(equipement)

#Pour chaque département ici l'exemple est le 05, la répartition de leur equipement
Df_dep = pd.DataFrame(Df.loc["05"]) #dfloc donne une series
Df_dep = Df_dep.iloc[:-2]
#print(Df_dep)
Df_dep.columns = ['values']
Df_dep['équipement'] = Df_dep.index
fig2 = px.pie(Df_dep, values="values", names="équipement", title='Répartition des équipements')
fig2.show()
